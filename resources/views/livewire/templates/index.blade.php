...
<div class="space-y-8">
    ...
    <!-- Header & Action Bar -->
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 md:p-8 flex flex-wrap items-center justify-between gap-6 shadow-2xl relative overflow-hidden backdrop-blur-xl">
    ...
    <div class="flex flex-wrap items-center justify-between gap-4 bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-xl backdrop-blur-md">
        <div class="flex items-center space-x-2 overflow-x-auto pb-1 md:pb-0">
            <button type="button" wire:click="$set('selectedCategory', 'all')" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ $selectedCategory === 'all' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'bg-slate-950 text-slate-400 hover:text-white hover:bg-slate-800' }}">
                All Templates
            </button>
            <button type="button" wire:click="$set('selectedCategory', 'landscape')" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ $selectedCategory === 'landscape' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'bg-slate-950 text-slate-400 hover:text-white hover:bg-slate-800' }}">
                Landscape 📐
            </button>
            <button type="button" wire:click="$set('selectedCategory', 'portrait')" class="px-4 py-2 rounded-xl text-xs font-bold transition {{ $selectedCategory === 'portrait' ? 'bg-indigo-600 text-white shadow-md shadow-indigo-600/20' : 'bg-slate-950 text-slate-400 hover:text-white hover:bg-slate-800' }}">
                Portrait 📱
            </button>
        </div>

        <div class="relative min-w-[240px]">
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search templates by name..." class="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-4 py-2 text-xs font-bold text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition">
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($schoolTemplates as $st)
                    ...
                    @endphp
                    <div class="bg-slate-900 border border-slate-800 hover:border-indigo-500/50 rounded-3xl p-5 shadow-xl hover:shadow-2xl hover:shadow-indigo-500/10 transition-all duration-300 flex flex-col justify-between group relative overflow-hidden">
                        <div>
                            <!-- Header Info -->
                            <div class="flex items-start justify-between mb-4">
                                ...
                                @if($isDefault)
                                    <span class="text-[10px] font-black text-emerald-400 bg-emerald-500/15 border border-emerald-500/30 px-2.5 py-1 rounded-full uppercase tracking-wider shrink-0 shadow-sm">
                                        Default ⚡
                                    </span>
                                @endif
                            </div>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <div class="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5">
                ...
                @if($assignSuccessMessage)
                    <div class="bg-emerald-950/90 border border-emerald-500/60 text-emerald-100 p-3.5 rounded-2xl flex items-center justify-between text-xs font-bold shadow-lg">
                        <div class="flex items-center space-x-2">
